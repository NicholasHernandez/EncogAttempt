...
public class PruneIncremental extends ConcurrentJob {
	...
	private final MarketMLDataSet evaluate;
	/**
	 ...
	 */
	public PruneIncremental(final MLDataSet training,final MarketMLDataSet evaluate,
			final NeuralNetworkPattern pattern, final long iterations,
	...
	public double calculateError(BasicNetwork network){
		...
		double pcnt = (double) correct / (double) count;
		return (1.0-pcnt);
	...
	public void performJobUnit(final JobUnitContext context) {
		...
		for (int z = 0; z < this.weightTries; z++) {
			...
			for (long i = 0; (i < this.iterations) && !getShouldStop()
					&& !strat.shouldStop(); i++) {
				train.iteration();
			}

			error = Math.min(error, this.calculateError(network));
		}
		...
		if (!getShouldStop()) {
			...
			reportStatus(
					...
					"Current: "
							...
							+ "; Best: "
							+ PruneIncremental
									.networkToString(this.bestNetwork) + " E:"+ this.low) ;
		}
