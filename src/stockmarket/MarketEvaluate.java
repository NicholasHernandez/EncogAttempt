...
public class MarketEvaluate {
	...
	public static void evaluate(File dataDir) {
		...
		double[] stor = new double[data.get(data.size()-1).getInput().getData().length];
		for(int i = 0; i< data.get(data.size()-1).getInput().getData().length-2;i++){
			//System.out.println(data.get(data.size()-1).getInput().getData()[51-i]);
			stor[i] = data.get(data.size()-1).getInput().getData()[i+1];
		}
		stor[51]= data.get(data.size()-1).getIdeal().getData(0);
		
		data.get(0).getInput().setData(stor);
		System.out.println(network.compute(data.get(0).getInput()).getData(0));
